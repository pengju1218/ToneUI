...
import com.csdn.view.tx.inter.PageIndicator;
...
public class IconTabPageIndicator extends LinearLayout implements PageIndicator {
    ...
    private int iconWidth,iconHeight,iconPadding,tab_textSize;
   ...
   public IconTabPageIndicator(Context context) {
    ...
    public IconTabPageIndicator(Context context, AttributeSet attrs) {
        ...
        tab_textSize = a.getDimensionPixelSize(R.styleable.TabView_tab_textSize, 0);
        a.recycle();
    ...
    private void addTab(int index, CharSequence text, int iconResId) {
        ...
        tabView.setTextSize(tab_textSize);
  ...
  /*  public void setIconWidthS(int[] sizes){

        final int tabCount = mTabLayout.getChildCount();
        for (int i = 0; i < tabCount; i++) {
            if(sizes.length<tabCount){
                return;
            }
            final TabView child = (TabView) mTabLayout.getChildAt(i);
            child.setIconHeight(sizes[i]);
            child.setIconWidth(sizes[i]);
        }
    }*/
    ...
    public void setCurrentItem(int item) {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (isSelected) {
                child.setTextColor(Color.parseColor("#1296db"));
                animateToTab(item);
            }else {
                child.setTextColor(Color.parseColor("#8a8a8a"));
            }
        }
    }
